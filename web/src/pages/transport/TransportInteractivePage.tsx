...
export default function TransportInteractivePage() {
  ...
  const filteredSchedules = useMemo(() => {
    ...
    return stats.schedules.filter((schedule) => statusFilter === 'all' || schedule.status === statusFilter);
  }, [stats, statusFilter]);
  ...
  const exportSchedules = () => {
    downloadCsv(
      `transport-schedules-${statusFilter}.csv`,
      filteredSchedules,
      [
        ...
        { key: 'departureTime', header: 'Giờ khởi hành' },
        {
          key: 'status',
          header: 'Trạng thái',
          accessor: (schedule) => transportStatusConfig[schedule.status].label,
        },
        ...
        { key: 'affiliateProfit', header: 'Lợi nhuận affiliate' },
        {
          key: 'seats',
          header: 'Ghế',
          accessor: (schedule) => `${schedule.occupiedSeats}/${schedule.totalSeats}`,
        },
      ]
    );
  };

  const copyScheduleCode = async () => {
    if (!selectedSchedule) {
      return;
    }

    await navigator.clipboard.writeText(selectedSchedule.code);
    setCopiedCode(selectedSchedule.code);
  ...
  return (
    <div className="space-y-12">
      <section>
        <div className="flex flex-col lg:flex-row justify-between items-start lg:items-end gap-6 mb-8">
          <div>
            ...
            <p className="text-on-surface-variant mt-3 max-w-2xl">
              Theo dõi lịch trình, khả năng lấp đầy ghế và doanh thu affiliate từ các nhà xe đang
          ...
          <div className="flex gap-3">
            <button
              ...
              className={`px-6 py-2.5 font-bold rounded-full shadow-sm transition-all flex items-center gap-2 border ${
                showFilters
                  ? 'bg-primary-container text-white border-primary-container'
                  : 'bg-surface-container-lowest text-on-surface border-outline-variant/10 hover:shadow-md'
              }`}
            >
              <span className="material-symbols-outlined text-lg">filter_list</span>
              <span>{showFilters ? 'Ẩn bộ lọc' : 'Lọc dữ liệu'}</span>
            </button>
            <button
              onClick={exportSchedules}
              className="px-6 py-2.5 bg-primary-container text-white font-bold rounded-full shadow-lg shadow-primary-container/20 flex items-center gap-2 hover:scale-[0.98] transition-transform"
            >
        ...
        {showFilters && (
          <div className="mb-6 bg-surface-container-low rounded-[2rem] p-3 flex flex-wrap gap-3 items-center">
            {[
              ...
              <button
                key={item.value}
                onClick={() =>
                  setStatusFilter(item.value as 'all' | AdminTransportSchedule['status'])
                }
            ...
            <button
              ...
              className="ml-auto px-5 py-2.5 rounded-full text-sm font-bold text-on-surface-variant hover:bg-white/60 transition-all"
            >
              Đặt lại
            </button>
          </div>
                ...
                {chartItems.map((item) => (
                  <div key={item.label} className="flex items-center gap-2">
                    ...
                    <span className="text-xs font-medium text-on-surface-variant">
                      {item.value} chuyến {item.label.toLowerCase()}
                    </span>
                  </div>
                ))}
              </div>
      ...
      <section className="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant/5 overflow-hidden">
        <div className="px-8 py-6 flex flex-col lg:flex-row justify-between gap-4 lg:items-center border-b border-outline-variant/10">
          <div>
            ...
            <p className="text-sm text-on-surface-variant mt-1">
              Đang hiển thị {filteredSchedules.length} chuyến phù hợp bộ lọc hiện tại
            </p>
          </div>
          <div className="flex items-center gap-3 text-xs text-outline italic">
            ...
            <span>Nhấn "Chi tiết" để xem nhanh từng chuyến</span>
          </div>
        </div>
          ...
          <table className="w-full text-left border-collapse">
            ...
            <tbody className="divide-y divide-outline-variant/10">
              {paginatedSchedules.map((schedule) => {
                ...
                return (
                  <tr key={schedule.id} className="hover:bg-surface-container-low/30 transition-colors group">
                    ...
                    <td className="px-8 py-6 text-center">
                      <span className={`px-3 py-1 rounded-full text-[10px] font-black uppercase ${status.badgeClass}`}>
                        {status.label}
                      </span>
                    </td>
                    ...
                    <td className="px-8 py-6 text-right">
                      <button
                        ...
                        className="px-4 py-2 rounded-full bg-surface-container-low text-on-surface text-xs font-bold hover:bg-surface-container transition-all"
                      >
                        Chi tiết
                      </button>
                    </td>
        ...
        <div className="px-8 py-6 border-t border-outline-variant/10 flex flex-col md:flex-row justify-between gap-4 md:items-center">
          <p className="text-xs text-outline font-medium">
            Hiển thị {(currentPageClamped - 1) * PAGE_SIZE + (paginatedSchedules.length ? 1 : 0)} - {(currentPageClamped - 1) * PAGE_SIZE + paginatedSchedules.length} trong tổng số {filteredSchedules.length} chuyến xe
          </p>
          <div className="flex items-center gap-2">
            <button
              onClick={() => setCurrentPage((page) => Math.max(1, page - 1))}
              disabled={currentPageClamped === 1}
              className="w-10 h-10 rounded-full border border-outline-variant/20 flex items-center justify-center hover:bg-surface-container-low transition-all disabled:opacity-40 disabled:cursor-not-allowed"
            >
      ...
      {selectedSchedule && (
        <section className="bg-surface-container-lowest rounded-xl shadow-sm border border-outline-variant/5 p-8">
          <div className="flex flex-col lg:flex-row justify-between gap-6">
            <div>
              <p className="text-[10px] uppercase tracking-[0.2em] font-bold text-primary mb-2">Chi tiết chuyến xe</p>
              <h3 className="text-2xl font-black text-on-surface">{selectedSchedule.code}</h3>
              <p className="text-on-surface-variant mt-2">{selectedSchedule.companyName}</p>
            </div>
            <div className="flex gap-3">
              <button
                onClick={copyScheduleCode}
              ...
              >
                {copiedCode === selectedSchedule.code ? 'Đã sao chép' : 'Sao chép mã'}
              </button>
              <button
                onClick={() => setSelectedSchedule(null)}
                className="px-5 py-2.5 rounded-full bg-primary-container text-white font-bold hover:brightness-105 transition-all"
              >
                Đóng
              </button>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mt-8">
            ...
            <div className="bg-surface-container-low rounded-2xl p-5">
              ...
              <p className="text-sm font-bold text-on-surface mt-2">
                {selectedSchedule.departureTime} • {selectedSchedule.departureDate}
              </p>
            </div>
            <div className="bg-surface-container-low rounded-2xl p-5">
            ...
            <div className="bg-surface-container-low rounded-2xl p-5">
              <p className="text-[11px] font-bold uppercase tracking-wider text-outline">Lợi nhuận</p>
              <p className="text-sm font-bold text-[#10B981] mt-2">{selectedSchedule.affiliateProfit}</p>
            </div>
          </div>
        </section>
      )}
      ...
      <section className="grid grid-cols-1 xl:grid-cols-12 gap-8">
        <div className="xl:col-span-8 bg-surface-container-lowest p-8 rounded-xl shadow-sm border border-outline-variant/5">
          <div className="flex justify-between items-center mb-6">
            <h4 className="text-sm font-black text-on-surface uppercase tracking-widest">Phân tích vận hành</h4>
        ...
        <div className="xl:col-span-4 space-y-4">
          <div className="bg-primary-container p-6 rounded-xl text-white shadow-xl shadow-primary-container/20 relative overflow-hidden">
            <div className="relative z-10">
              ...
              <h4 className="text-lg font-bold mb-3">Tỷ lệ lấp đầy trung bình {stats.averageOccupancyRate.toFixed(1)}%</h4>
              <p className="text-sm leading-relaxed text-white/85">
                Dữ liệu ghế đang được tổng hợp trực tiếp từ các lịch trình đã đồng bộ cùng trạng thái chỗ ngồi.
              </p>
            </div>

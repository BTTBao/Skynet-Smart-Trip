      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6">
        <div>
          ...
          <p className="text-on-surface-variant mt-3 max-w-2xl">
            Theo dõi giao dịch chuyến đi, tình trạng thanh toán và tốc độ tăng trưởng khách
            hàng mới trên toàn hệ thống.
          </p>
        </div>
        <div className="flex items-center gap-3 bg-surface-container-low p-1.5 rounded-full">
          <button className="px-6 py-2.5 bg-white shadow-sm rounded-full text-sm font-bold text-on-surface">
            Tất cả ({stats.totalBookings})
          </button>
          <button className="px-6 py-2.5 hover:bg-white/50 rounded-full text-sm font-medium text-on-surface-variant transition-all">
            Đã thanh toán ({stats.paidBookings})
          </button>
          <button className="px-6 py-2.5 hover:bg-white/50 rounded-full text-sm font-medium text-on-surface-variant transition-all">
            Chờ xử lý ({stats.pendingBookings})
          </button>
        </div>
      </div>
      ...
      <div className="bg-white rounded-xl shadow-[0px_40px_80px_rgba(21,28,39,0.04)] overflow-hidden border border-surface-container-high/40">
        <div className="px-10 py-8 border-b border-surface-container flex items-center justify-between">
          <div>
            ...
            <p className="text-sm text-on-surface-variant mt-1">
              Hiển thị các booking mới nhất được đồng bộ từ cơ sở dữ liệu
            </p>
          </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-surface-container-low/50">
                <th className="px-10 py-5 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">
                  Tên người dùng
                </th>
                <th className="px-10 py-5 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest">
                  Điểm đến
                </th>
                <th className="px-10 py-5 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest text-right">
                  Tổng tiền
                </th>
                <th className="px-10 py-5 text-[11px] font-bold text-on-surface-variant uppercase tracking-widest text-center">
                ...
                return (
                  <tr key={booking.id} className="hover:bg-surface-container-low/30 transition-all">
                    ...
                    <td className="px-10 py-6 text-center">
                      <span
                        className={`inline-flex items-center px-4 py-1.5 rounded-full text-[11px] font-bold tracking-wide ${status.className}`}
                      >
                        <span
                          className={`w-1.5 h-1.5 rounded-full mr-2 ${status.dotClass}`}
                        ></span>
                        {status.label}
                      </span>
                    </td>
        ...
        <div className="px-10 py-8 bg-surface-container-low/20 flex items-center justify-between">
          <p className="text-xs text-on-surface-variant font-medium">
            Hiển thị <span className="text-on-surface font-bold">{stats.bookings.length}</span>{' '}
            trên <span className="text-on-surface font-bold">{stats.totalBookings}</span> giao
            dịch
          </p>
          <div className="flex items-center gap-2">
            <button className="w-10 h-10 rounded-full border border-surface-container flex items-center justify-center hover:bg-white transition-all">
              <span className="material-symbols-outlined text-sm">chevron_left</span>
            </button>
            <button className="w-10 h-10 rounded-full bg-primary text-white flex items-center justify-center font-bold text-xs shadow-md shadow-primary/20">
              1
            </button>
            <button className="w-10 h-10 rounded-full border border-surface-container flex items-center justify-center hover:bg-white transition-all text-xs font-bold">
              2
            </button>
            <button className="w-10 h-10 rounded-full border border-surface-container flex items-center justify-center hover:bg-white transition-all text-xs font-bold">
              3
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-10 mt-10 opacity-70">
        <div className="text-left">
          ...
          <p className="text-xs max-w-xs leading-relaxed">
            Dữ liệu thanh toán được đồng bộ theo thời gian thực từ giao dịch chuyến đi và
            trạng thái xử lý booking của khách hàng.
          </p>
        </div>
